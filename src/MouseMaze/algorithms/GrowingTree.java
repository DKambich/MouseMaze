...
public class GrowingTree extends MazeGenerator {
  ...
  public void stepMaze(){
    if(allVisited()) {
      //If all the cells have been visited the maze is complete, clear the rest of the cells
  ...
  public Cell selectCell(ArrayList<Cell> cellList) {
    //Choose between selecting from the end of the cell list(similar to Backtracker) or from a random point in the list(similar to Prim)
    return (Math.random() <= primPercentage) ? cellList.get((int) (Math.random() * cellList.size())) : cellList.get(cellList.size() - 1);
  }
