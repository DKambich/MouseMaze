...
public class Kruskal extends MazeGenerator {
  ...
  public void stepMaze() {
    if (!edges.isEmpty()) {
      ...
      neighbor = (nextEdge.direction % 2 == 0) ? maze.cells[nextEdge.row - 1][nextEdge.col]
          : maze.cells[nextEdge.row][nextEdge.col - 1];
      if ((!sets[current.getRow()][current.getCol()].isConnected(sets[neighbor.getRow()][neighbor.getCol()]))) {
        // If the set at the current cell and the set at the neighbor cell are not
        // connected...
