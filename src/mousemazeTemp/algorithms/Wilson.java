...
public class Wilson extends MazeGenerator {
    ...
    public void startMaze(int width, int height, int numCells) {
        ...
        cellDir = new int[maze.cells.length][maze.cells[0].length];
        unvisited = new ArrayList<Cell>();

        maze.cells[(int) (Math.random() * maze.cells.length)][(int) (Math.random()
                * maze.cells[0].length)].visited = true;
    ...
    public Cell getCellFromDirection(Cell current, int direction) {
        return (direction % 2 == 0) ? getCell(current.getRow() + directions[direction], current.getCol())
                : getCell(current.getRow(), current.getCol() + directions[direction]);
    }

    public int getDirection(Cell from, Cell to) {
        return (from.getCol() - to.getCol() != 0) ? ((from.getCol() - to.getCol() == 1) ? 3 : 1)
                : ((from.getRow() - to.getRow() == 1) ? 0 : 2);
    }
}
