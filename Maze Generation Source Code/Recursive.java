...
public class Recursive extends MazeGenerator
{
  ...
  public void startMaze(int width, int height, int numCells){
    ...
    commands.add(new RecursionCommand(0, 0, numCells, numCells, ((int) (Math.random() * 2) == 0) ? HORIZONTAL : VERTICAL));
  }
  ...
  public class RecursionCommand {
    ...
    public RecursionCommand[] runCommand() {
      ...
      if(orientation == HORIZONTAL) {
        ...
        int topHeight = ++walledRow - y, bottomHeight = height - topHeight; //++walledRow changes row from an index to a "height" value
        return new RecursionCommand[]{ new RecursionCommand(x, y, width, topHeight, selectOrientation(width, topHeight)), new RecursionCommand(x, walledRow, width, bottomHeight, selectOrientation(width, bottomHeight))};
      }
      else {
        ...
        int leftWidth=++walledCol-x, rightWidth=width-leftWidth; //++walledCol changes col from an index to a "width" value
        return new RecursionCommand[]{new RecursionCommand(x, y, leftWidth, height, selectOrientation(leftWidth, height)),new RecursionCommand(walledCol, y, rightWidth, height, selectOrientation(rightWidth, height))} ;
      }
    }
    ...
    public void addWall(Cell a, Cell b){
      switch(a.getCol()-b.getCol()){
        case  1: a.pointers[3]=null;  b.pointers[1]=null; break;
        case -1: a.pointers[1]=null;  b.pointers[3]=null; break;
      }
      switch(a.getRow()-b.getRow()){
        case  1:  a.pointers[0]=null;  b.pointers[2]=null; break;
        case -1:  a.pointers[2]=null;  b.pointers[0]=null; break;
      }
    }
  }
}
